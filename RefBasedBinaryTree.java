...
public class RefBasedBinaryTree implements BinaryTree {
    ...
    private boolean find(Integer value, TreeNode cur){
        if(cur == null){
            return false;
        ...
        return find(value, cur.getLeft()) || find(value, cur.getRight());
    }
    ...
    private int getMax(TreeNode cur, int max){
        if(cur == null){
            return max;
        }
        ...
        if(getMax(cur.getLeft(), max) > getMax(cur.getRight(), max)){
            return getMax(cur.getLeft(), max);
        } else {
            return getMax(cur.getRight(), max);
        }
    }
